//! Rust FFI bindings for aviate_gz_bridge
//!
//! This module provides safe Rust wrappers around the C interface to
//! access gz-sim physics data via shared memory.
//!
//! # Usage
//!
//! ```rust,ignore
//! use aviate_gz_plugin::GzBridge;
//!
//! let bridge = GzBridge::new()?;
//! loop {
//!     if let Some(state) = bridge.get_model_state() {
//!         println!("Position: {:?}", state.pos);
//!     }
...
/// Model state from gz-sim (all values in SI units)
#[repr(C)]
...
impl std::fmt::Display for GzBridgeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::NotInitialized => write!(f, "GzBridge not initialized"),
...
/// Safe wrapper around the gz-sim bridge
///
...
impl GzBridge {
    /// Create a new GzBridge connection
    ///
    /// Returns an error if the AviateGzPlugin is not running in Gazebo.
    pub fn new() -> Result<Self, GzBridgeError> {
        let result = unsafe { aviate_gz_init() };
        match result {
            0 => Ok(Self { initialized: true }),
            -1 => Err(GzBridgeError::PluginNotRunning),
            _ => Err(GzBridgeError::NotInitialized),
        }
    }

    /// Try to connect to the bridge, retrying if plugin not ready
    pub fn connect_with_retry(max_attempts: u32, delay_ms: u64) -> Result<Self, GzBridgeError> {
    ...
    pub fn set_motor_speeds(&self, velocities: &[f64]) -> Result<(), GzBridgeError> {
        if !self.initialized {
            return Err(GzBridgeError::NotInitialized);
        }
        ...
        if result == 0 {
            ...
            Err(GzBridgeError::MotorCommandFailed)
        }
    }
...
pub fn enu_to_ned(enu: [f64; 3]) -> [f64; 3] {
    [enu[1], enu[0], -enu[2]]  // [north, east, down]
}
...
mod tests {
    ...
    fn test_enu_to_ned() {
        ...
        assert_eq!(ned, [2.0, 1.0, -3.0]);  // 2m north, 1m east, 3m down (negative)
    }
