use aviate_core::hal::{SensorHal, ActuatorHal, SystemHal, AviateHal, CommandHal, SystemCommand, TelemetryHal};
...
impl SitlHal {
    ...
    pub fn set_command(&mut self, cmd: SystemCommand) {
...
impl CommandHal for SitlHal {
    fn recv_command(&mut self) -> Option<SystemCommand> {
        self.command.take()
    }
}

impl TelemetryHal for SitlHal {
    fn send_telemetry(&mut self, _state: &StateEstimate) {}
}
