...
pub fn seqlock_read<T, F: FnMut() -> T>(seq: &AtomicU32, mut copy_payload: F) -> Option<T> {
    for _ in 0..SEQLOCK_MAX_RETRIES {
        ...
        // payload copy from sinking BELOW the second `seq` read, and
        // a weakly-ordered CPU will do exactly that — the copy then
        // reads a later write while both `seq` reads agree, and a
        // torn snapshot escapes. The Acquire FENCE is the barrier
        // that orders the copy ahead of the re-read (the same
        // `smp_rmb()` Linux's `read_seqretry()` issues).
