use crate::types::{Scalar, MetersPerSecond, Normalized, FloatExt};
...
impl VelocityController {
    ...
    pub fn step(
    ...
    ) -> (Normalized, Quaternion) { // Returns collective thrust and attitude setpoint (roll/pitch)
        let error = Vector3 {
        ...
        let roll_pitch_quat = Quaternion::from_axis_angle(Vector3::new(1.0, 0.0, 0.0), roll_sp_clamped) // Roll rotation
            .mul(&Quaternion::from_axis_angle(Vector3::new(0.0, 1.0, 0.0), pitch_sp_clamped)); // Pitch rotation
