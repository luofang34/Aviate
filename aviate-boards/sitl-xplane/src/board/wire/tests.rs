...
fn airborne(wire: &mut WireConstraints) {
    ...
    wire.constrain(&mut outputs, 4, Some(20.0), 0.01);
}
...
fn the_collective_mean_rises_no_faster_than_the_band_allows() {
    ...
    for _ in 0..50 {
        ...
        wire.constrain(&mut step, 4, Some(20.0), 0.01);
        let now = mean(&step);
...
fn the_mean_ceiling_holds_and_the_bookkeeping_is_the_wire() {
    ...
    for _ in 0..2000 {
        outputs = [1.0f32; 16];
        wire.constrain(&mut outputs, 4, Some(20.0), 0.05);
    }
...
fn a_railed_lane_stays_under_the_stall_ceiling_with_its_moment_direction_kept() {
    ...
    for _ in 0..2000 {
        ...
        wire.constrain(&mut warm, 4, Some(20.0), 0.05);
    }
    let mut outputs = [0.0f32; 16];
...
fn arming_before_the_first_fix_still_finds_its_ground() {
    ...
    for _ in 0..80 {
        ...
        wire.constrain(&mut warm, 4, Some(7.0), 0.05);
    }
    ...
    wire.constrain(&mut climb, 4, Some(8.0), 0.05);
    let mut aloft = [0.0f32; 16];
    aloft[..4].copy_from_slice(&[0.2, 0.4, 0.2, 0.4]);
    wire.constrain(&mut aloft, 4, Some(8.0), 0.05);
