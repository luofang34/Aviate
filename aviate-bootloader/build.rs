...
fn generate_memory_x(
    ...
    if family == ChipFamily::Rp2350 {
        ...
        // Emit the FLASH length in exact bytes, not truncated kibibytes:
        // the `/1024` form dropped the final 768 bytes (63K, ending at
        // 0x1000_fd00) and let the log claim a 64K region it did not
        // build. With the exact length the region ends precisely at the
...
/* End of the bootloader FLASH region. scripts/check_bootloader_images.sh
   reads this symbol and fails if it exceeds the application start, so the
   linker geometry itself — not just the current image footprint — is
   proven to stay out of the application partition. */
