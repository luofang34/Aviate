...
//!
//! Generates `memory.x` from probe-rs device geometry and explicit bootloader
//! partition limits.
...
/// RP2350 bootloader/application partition boundary. The application is linked
/// here, and the bootloader reads its vectors here during handoff. The generated
/// bootloader FLASH region must not extend past this address.
///
/// A build script cannot access constants through the target dependency graph.
/// Artifact verification therefore bounds the generated linker region by
/// `aviate_chip_rp2350::APP_START` and requires the test-app origin to
/// equal that authoritative handoff address.
...
fn get_ram_region(
    ...
    let region = match selector {
        ...
        RamSelector::AtAddress(addr) => {
            // Prefer the requested bank, but remain usable with registry entries
            // that expose the same RAM under a different base address.
            ram_regions
                .iter()
...
fn generate_memory_x(
    ...
    // RP2350 needs special handling: the vector table comes first at the
    // flash base, and the Image Definition (start_block) follows it.
    if family == ChipFamily::Rp2350 {
        // Preserve the byte-exact capped length. Kibibyte rounding could shrink
        // the linker region below the selected device/partition limit.
        format!(
            r#"/* Auto-generated by build.rs from probe-rs registry - DO NOT EDIT */
/* RP2350 Boot ROM reads the initial SP/PC from the vector table at the
   flash base. The Boot ROM scans the first 4 KiB of flash for the Image
   Definition (start_block). This layout matches rp235x-hal. */
MEMORY
{{
    FLASH : ORIGIN = {:#010x}, LENGTH = {:#x}
    RAM   : ORIGIN = {:#010x}, LENGTH = {}K
}}

/* Expose the actual linker-owned FLASH region end for independent
   verification against the application partition boundary. */
__aviate_bootloader_flash_region_end = ORIGIN(FLASH) + LENGTH(FLASH);

/* Place start_block directly after the vector table */
SECTIONS {{
    .start_block : ALIGN(4)
    {{
        __start_block_addr = .;
...
/* Move .text after the start_block. cortex-m-rt PROVIDEs _stext at the end
   of .vector_table, so .text would overlap .start_block without this. */
...
fn main() -> Result<()> {
    ...
    println!(
        "cargo:warning=Generated memory.x for {}: bootloader flash window={:#x}+{:#x}, RAM={:#x}+{}K",
        config.target_name,
        flash_base,
        flash_size.min(config.bootloader_flash_limit as u64),
        ram_base,
        ram_size / 1024,
    );
